...
contract BiequityCore is Ownable {
  ...
  /// @notice The ERC-3643 identity registry shared across all tokens.
  ...
  constructor(
    ...
    // Deploy the shared identity registry (ERC-3643 compliance layer).
    // All BiequityTokens deployed by this core will reference this registry.
    IdentityRegistry registry = new IdentityRegistry();
    identityRegistry = registry;

    // Deploy factory — passes identity registry so tokens can enforce compliance.
    FACTORY = new BiequityTokenFactory(address(this), address(registry));
  }
  ...
  function settleTokens(
    ...
    require(stock.totalIssued >= stock.totalBacked + amount, "Exceeds issued");
  ...
  function redeem(string calldata symbol, uint256 tokenAmount) external {
    ...
    require(
      stock.totalBacked >= (stock.totalIssued * stock.minBackedRatio) / 100,
      "Not fully backed"
    );
  ...
  /// @notice Whitelist an investor to hold biequity tokens.
  /// @dev Proxies to the shared IdentityRegistry. Only callable by owner.
  function whitelistInvestor(address investor) external onlyOwner {
    IdentityRegistry(address(identityRegistry)).addVerifiedInvestor(investor);
  }

  /// @notice Remove an investor from the whitelist.
  function removeInvestor(address investor) external onlyOwner {
    IdentityRegistry(address(identityRegistry)).removeVerifiedInvestor(
      investor
    );
  }

  /// @notice Batch whitelist investors (gas-efficient onboarding).
  function batchWhitelistInvestors(
    address[] calldata investors
  ) external onlyOwner {
    IdentityRegistry(address(identityRegistry)).batchAddVerifiedInvestors(
      investors
    );
  }
  ...
  function isVerified(address investor) external view returns (bool) {
    return identityRegistry.isVerified(investor);
  }
  ...
  function withdrawUsdcFromStock(
    ...
    StockConfig storage stock = stocksByToken[tokenAddr];
    require(address(stock.tokenAddress) != address(0), "Stock not registered");
